 ...
 * Filename: IT151IP5.java
 * Created:  11/06/15 by Duane Osburn
 * Modified: 11/06/15 by Duane Osburn
 * 
...
//Declare attributes
    ...
    private double HDAvgSales;
    private double HDShipChrg;
//Set Constructor    
    public HardDrive (int iProdId, String iType, String iClass, String iSize, String iAccessSp, double iPrice, String iVendor, double iAvgSales,
            double iShipChrg){
		...
		HDPrice = iPrice;
		HDVendor = iVendor;
                HDAvgSales = iAvgSales;
                HDShipChrg = iShipChrg;
		}
//Mutators
	...
	public void setVendor(String setVendor)
	{
        ...
        public void setAvgSales (double setAvgSales)
        {
                HDAvgSales = setAvgSales;
        }
        public void setShipChrg (double setShipChrg)
        {
                HDShipChrg = setShipChrg;    
        }
//Accessors
	...
	public String getVendor()
	{
        ...
        public double getShipChrg()
        {
                return HDShipChrg;
        }
}//end class HardDrive
//
public class IT151IP5 {
   ...
   //id number 
    ...
    public static void main(String[] args) {
      ...
      String cClassOut;
      int cIDOut;
      Double cPriceOut, cAvgSalesOut, cShipOut;
      //Declare Array for class objects
      HardDrive[] cUser = new HardDrive[2];
      //Display opening interaction message
      openingMsg = "Welcome to the Hard Drive On-Line Ordering System\n";
      JOptionPane.showMessageDialog(null, openingMsg);
      //use for loop to build array of class items
      for (int i=0; i<2; i++){
        openingMsg = "Order #"+(i+1)+ "\n";
        JOptionPane.showMessageDialog(null, openingMsg);   
      ...
      //Build Class
         cUser[i] = new HardDrive(prodIDin,null,prodClassin,null,null,prodPricein,null,prodSalesAvgIn,prodShipIn);
      //
         clType = "plate";
         clSize = "1TB"; 
         clAccsp = "7500//16ms"; 
         clVendor = "Kingston";
         cUser[i].setType(clType);
         cUser[i].setSize(clSize);
         cUser[i].setAccess(clAccsp);
          cUser[i].setVendor(clVendor);
      //
      }
      // build output strings
      // use a for loop to print the array items
      for (int i=0; i<2; i++){
      //Get Class Information
        cIDOut = cUser[i].getProdId();
        cClassOut = cUser[i].getHClass();
        cPriceOut = cUser[i].getPrice();
        cAvgSalesOut = cUser[i].getAvgSales();
         cShipOut = cUser[i].getShipChrg();
      //Set Output Messages
        ...
        prodAvgSalesout = "The Average Sales for this product is: " + cAvgSalesOut + ".\n";
        prodShipChrgout = "The total shipping charges for this product is: " + cShipOut + ".\n";
